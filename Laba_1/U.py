...
# --------------------------------------------------------------------------------
a, b = 30, 1500  # Границы интервала

# --------------------------------------------------------------------------------
#  1. ЧИСЛЕННЫЙ РАСЧЕТ M[T] (МАТЕМАТИЧЕСКОГО ОЖИДАНИЯ)
# --------------------------------------------------------------------------------
def expected_value_uniform(a, b, steps=1000):
    """
    ...
    for i in range(steps):
        ...
        integral_sum += t_mid * pdf_value
    return integral_sum * dt

# --------------------------------------------------------------------------------
#  2. ЧИСЛЕННЫЙ РАСЧЕТ ВТОРОГО МОМЕНТА M[T^2]
# --------------------------------------------------------------------------------
def second_moment_uniform(a, b, steps=1000):
    """
    ...
    for i in range(steps):
        t_mid = a + (i + 0.5) * dt
        pdf_value = 1.0 / (b - a)
        integral_sum += (t_mid ** 2) * pdf_value
    return integral_sum * dt
...
def pdf(t):
    """Плотность для U(a,b)"""
    return 1/(b - a) if (a <= t <= b) else 0.0
...
# --------------------------------------------------------------------------------
mean_time = expected_value_uniform(a, b)
mean_square_time = second_moment_uniform(a, b)
